...
#cp run-query2.sh ${RESULT_DIR}/
RESULT_FILE=${RESULT_DIR}/exp-`date +"%H-%M"`-q25.csv
DATASET_DIR=${ROOT_DIR}/tmp/csv/q25/
HPAT_CGEN_BINARY_DIR=${ROOT_DIR}/.julia/v0.5/HPAT/examples/queries_devel/q25/
HPAT_DATAGEN_DIR=${ROOT_DIR}/.julia/v0.5/HPAT/examples/queries_devel/q25/
...
for dataset_factor in "20" ; do
    ...
    table2_path=$DATASET_DIR/web_sales_sanitized_${dataset_factor}f.csv
    table1_rows=`cat $table1_path | wc -l`
    table2_rows=`cat $table2_path | wc -l`
    if [ ! -f ${table1_path} ] || [ ! -f ${table2_path} ]; then
        echo "Dataset files do not exist !!!"
	exit
    fi
    echo ":D Running Spark"
    ${SPARK_DIR}/sbin/start-all.sh
    # Psephi08 master
    ${SPARK_DIR}/bin/spark-submit --conf spark.sql.autoBroadcastJoinThreshold=-1 --jars /home/whassan/commons-csv-1.1.jar,/home/whassan/spark-csv_2.10-1.4.0.jar --class Query25 $SPARK_QUERY_DIR/target/scala-2.10/query26_2.10-0.1.jar $table1_path $table2_path &> tmp_spark.txt


    time_q25_spark=`cat tmp_spark.txt  | grep '\*\*\*\*\*\*' | cut -d ' ' -f 6`
    echo "Time took for Query 25[Spark]: "$time_q25_spark
    ${SPARK_DIR}/sbin/stop-all.sh
    #rm tmp_spark.txt

    echo ":D Copying to hdf5"
    julia ${HPAT_DATAGEN_DIR}/create_data_test.jl 1 "$table1_path" "$table2_path" 
    ...
    time_q25_hpat_config1=`cat tmp_hpat_config1.txt  | grep '\*\*\*\*\*\*' | cut -d ' ' -f 6`
    echo "Time took for Query 25[Hpat]: "$time_q25_hpat_config1
    ...
    echo "$dataset_factor,$table1_rows,$table2_rows,$time_q25_spark,$time_q25_hpat_config1" >>  ${RESULT_FILE}
