...
for dataset_factor in 500 ; do
    ...
    for i in $(seq ${NUM_MAP_TASKS}); do
        ...
        sed -i 's/|/,/g' ${DATASET_PATH}/store_sales_${i}_${dataset_factor}f.dat && cat ${DATASET_PATH}/store_sales_${i}_${dataset_factor}f.dat | cut -d, -f3,4 > ${DATASET_PATH}/store_sales_sanitized_${i}_${dataset_factor}f.csv

        sed -i 's/|/,/g' ${DATASET_PATH}/item_${i}_${dataset_factor}f.dat && cat ${DATASET_PATH}/item_${i}_${dataset_factor}f.dat | cut -d, -f1,10,13 |  sed 's/[.!@#$%^&*()-]//g' | sed -e 's,_,,g' | tr abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ 012345678901234567890123456789  | cut -d ' ' -f1  > ${DATASET_PATH}/item_sanitized_${i}_${dataset_factor}f.csv
    done
