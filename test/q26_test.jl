...
@acc hpat function q26(category, item_count, num_centroids, file_name)
    ...
    sale_items_fil = sale_items[:i_category==category]

    customer_i_class = aggregate(sale_items_fil, :ss_customer_sk, :ss_item_count = length(:ss_item_sk),
    ...
    customer_i_class_fil = customer_i_class[:ss_item_count>item_count]
    return customer_i_class_fil[:ss_item_count],customer_i_class_fil[:id3]
end
